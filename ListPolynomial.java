...
public class ListPolynomial extends AbstractPolynomial {
    ...
    public void addTerm(int power, double coefficient) throws Exception {
        ...
        for(PolynomialTerm p:termList){
            ...
            else if(power==p.getPower()){
                double newCoefficientValue=p.getCoefficient()+coefficient;
                termList.remove(index);
                if(newCoefficientValue!=0) {
                    termList.add(index, new PolynomialTerm(newCoefficientValue, power));
                    return;
                }
            }
            index++;
    ...
    public double evaluate(double point) {
        ...
        double ans=0;
        for(PolynomialTerm p: termList){
            ans+=p.getCoefficient()*Math.pow(point,p.getPower());
        }
        return ans;
    }
    ...
    public Polynomial add(Polynomial p) {
        ...
        int maxLengthOfResultPolynomial=Math.max(this.degree(),p.degree());
        for(int i=maxLengthOfResultPolynomial;i>=0;i--){
            double valueOfCurr=(this.coefficient(i)!=0?this.coefficient(i):0);
            double valueOfNew=(p.coefficient(i)!=0?p.coefficient(i):0);
            if(valueOfCurr!=0||valueOfNew!=0){
                try {
                    result.addTerm(i, valueOfCurr + valueOfNew);
                }
                catch(Exception E){
    ...
    public Polynomial subtract(Polynomial p) {
         ...
         */
        Polynomial result= new ListPolynomial();
        int maxLengthOfResultPolynomial=Math.max(p.degree(),this.degree());
        for(int i=maxLengthOfResultPolynomial;i>=0;i--){
            double valueOfCurr=(this.coefficient(i)!=0?this.coefficient(i):0);
            double valueOfNew=(p.coefficient(i)!=0?p.coefficient(i):0);
            if(valueOfCurr!=0||valueOfNew!=0){
    ...
    public Polynomial multiply(Polynomial p) {
        ...
        for(PolynomialTerm term: this.termList) {
            Iterator<PolynomialTerm> it = p.getIterator();
            while(it.hasNext()){
                PolynomialTerm itTerm=it.next();
                double coeff=itTerm.getCoefficient();
                int power=itTerm.getPower();
                try {
                    result.addTerm(term.getPower() + power, term.getCoefficient() * coeff);
                }
                catch(Exception E){
